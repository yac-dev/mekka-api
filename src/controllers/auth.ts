...
interface CustomRequestType<T> extends Request {
  body: T;
}

// でもやっぱ、ここってuser modelと密接に関わっているし。これやっぱ共通化した方がいいな。methodの部分はまた別でextendする感じにする必要があるかな。
interface User {
  _id: string;
  name: string;
  email: string;
...
interface CustomResponseType<T> extends Response {
  json: (body: T) => this;
}

export const signup = async (
  request: CustomRequestType<SignupRequestBodyType>,
  response: CustomResponseType<SignupResponseType>,
  next: NextFunction
) => {
